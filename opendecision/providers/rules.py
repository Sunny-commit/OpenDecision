"""Deterministic rules provider for high-confidence and fallback decisions."""
...
class Rule(BaseModel):
    ...
    def matches(self, state: Mapping[str, Any]) -> bool:
        ...
        if self.operator == "contains":
            return str(self.value).casefold() in str(actual).casefold()
        if self.operator == "in":
            return bool(actual in self.value)
        if self.operator == "regex":
            return re.search(str(self.value), str(actual), re.IGNORECASE) is not None
        return False
    ...
    def predict(
        ...
        for rule in self.rules:
            if rule.matches(payload):
                return {
                    "answer": _answer(question, rule.decision, rule.confidence),
                    "raw": {"rule": rule.model_dump()},
                }
        if self.default is None:
            raise LookupError("no deterministic rule matched")
...
def _answer(
    ...
    if question.type == "choice":
        ...
        if str(decision) not in labels:
            raise ValueError(f"rule decision {decision!r} is not a declared option")
        remaining = (1.0 - confidence) / max(len(labels) - 1, 1)
        probabilities = {
            label: (confidence if label == decision else remaining) for label in labels
        }
        return {
            "choice": str(decision),
    ...
    if question.type == "score":
        return {"score": float(decision), "confidence": confidence}
    probability = confidence if bool(decision) else 1.0 - confidence
