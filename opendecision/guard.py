...
class DecisionGuard:
    ...
    def decide(self, state: Mapping[str, Any] | str, question: DecisionQuestion | Mapping[str, Any] | str) -> DecisionResult:
    ...
    def _normalize(self, answer: Mapping[str, Any], raw_output: Any, question: DecisionQuestion) -> DecisionResult:
        ...
        if question.type == "choice":
            ...
            return DecisionResult(decision=str(decision), probabilities=probabilities, confidence=confidence, question_type="choice", provider=self.provider.name, metadata={"raw_answer": dict(answer)}, raw_output=raw_output)
        if question.type == "score":
            ...
            confidence = _as_probability(_first(answer, "confidence"))
            return DecisionResult(decision=float(score), probabilities=distribution, confidence=confidence, question_type="score", provider=self.provider.name, metadata={"raw_answer": dict(answer)}, raw_output=raw_output)
        probability = _as_probability(_first(answer, "noul", "probability", "confidence", "decision"))
        if probability is None:
            raise ValueError("provider response did not contain a noul probability")
        decision = probability >= question.threshold
        return DecisionResult(decision=decision, probabilities={"true": probability, "false": 1.0 - probability}, confidence=max(probability, 1.0 - probability), question_type="noul", provider=self.provider.name, metadata={"threshold": question.threshold, "raw_answer": dict(answer)}, raw_output=raw_output)
